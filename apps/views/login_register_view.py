...
# 注册路由
# 注册
# @reg_bp.route("/", methods=["POST", "GET"], strict_slashes=False)  # 在Windows下增加strict_slashes=False 这样不会再 严格的要求 斜杠
@reg_bp.route("/", methods=["GET", "POST"], strict_slashes=False)
def reg_index():
    try:
        ...
        User.add_db_data(User(username=username, password=password))
